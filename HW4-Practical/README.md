...
مستندات Swagger نیز در آدرس‌های زیر ارائه می‌شوند:
...
فایل [swagger.yaml](./swagger.yaml) براساس OpenAPI 3.1 نوشته شده و برای عملیات ایجاد، فهرست، دریافت، ویرایش، تغییر وضعیت و حذف کار و همچنین دریافت برچسب‌ها نمونه قابل‌اجرای GraphQL دارد. در Swagger UI از منوی Examples نمونه موردنظر را انتخاب کرده و سپس از گزینه Try it out استفاده کنید.
...
تست‌ها چرخه کامل ایجاد، دریافت، فیلتر، ویرایش، تغییر وضعیت و حذف را به همراه مدیریت رابطه برچسب‌ها و خطاهای اعتبارسنجی بررسی می‌کنند. علاوه بر آن، تست مرزهای معماری اجازه نمی‌دهد لایه‌های Domain و Application به فریم‌ورک‌ها، SQLite یا لایه‌های بیرونی وابسته شوند و Use Caseها نیز با Repository ساختگی به‌صورت مستقل تست می‌شوند. تست‌های Swagger نیز ساختار و Referenceهای فایل OpenAPI، ارائه شدن فایل و رابط Swagger UI و اجرای واقعی تمام مثال‌های مستندشده به‌شکل یک گردش‌کار کامل را کنترل می‌کنند.
